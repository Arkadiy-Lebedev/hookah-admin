...
const activeTableForAdmin = async (tableId: number) => {
    let date = new Date()
    let dateBusy = new Date(props.tableSingle.tables.filter(el => el.status == 'busy')[0]?.timeStart)
...
{{ isStatusActive }} -->
    <Dialog header="Товары" v-model:visible="isModalProducts"
        :breakpoints="{ '1420px': '60vw', '960px': '80vw', '700px': '90vw', '640px': '99vw' }" :style="{ width: '60vw', 'height':'90vh' }"
        :modal="true">
        <ProductsList :id-booking="idBooking" @closeModal="closeModal"></ProductsList>
    </Dialog>
    <div class="booking__btn-group">
         <Button :disabled="new Date().toLocaleDateString() !== new Date(dateCalendar).toLocaleDateString()" v-if="!isStatusActive" @click="activeTableForAdmin(tableInfoSingle[0].id)" label="Посадка" severity="success"
    ...
    <div class="error-message">
                    <InlineMessage v-if="errors.activebusy.error" severity="error">{{ errors.activebusy.text }}</InlineMessage>
                </div>
            <div class="datapicker__group hidden">
                    ...
                    <div class="error-message">
                        <InlineMessage v-if="errors.timebusy.error" severity="error">{{ errors.timebusy.text }}</InlineMessage>
                    </div>
            ...
            <div v-if="tableSingle.tables.filter(el => el.status == 'busy').length" class="orders">
                ...
                <ul v-for="(item, index) in tableSingle.tables.filter(el => el.status == 'busy')" :key="item.id"
                    ...
                    <li class="list__item flex align-items-center py-2 border-bottom-1 surface-border">
                        ...
                        <span class="text__booking text-900 line-height-3"><span class="text-blue-500">{{
                            item.timeStart.split(' ')[1]
                        }}</span> {{ item.phone }} {{ item.client_name }} {{ item.order_client }}
                        ...
                        <div class="btn__group">
                            ...
                            <Button @click="deleteBooking(item.id)" icon="pi pi-times" severity="danger" rounded
                                aria-label="Bookmark" size="small" />
